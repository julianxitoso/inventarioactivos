...
ini_set('memory_limit', '512M'); // PREVIENE PANTALLAS BLANCAS POR FALTA DE RAM
set_time_limit(300);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validar que PHP sí haya recibido el archivo (A veces el hosting lo bloquea por peso)
    if (isset($_FILES['archivo_excel']) && $_FILES['archivo_excel']['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            $_SESSION['import_error_message'] = "Error en la subida. Código interno de PHP: " . $file['error'];
        }
    } else {
        $_SESSION['import_error_message'] = "El servidor bloqueó el archivo. Es probable que sea demasiado pesado para la configuración actual de tu hosting (revisa post_max_size en cPanel).";
    }
} else {
    $_SESSION['import_error_message'] = "Acceso denegado. Debe enviar el formulario.";
}

// REDIRECCIÓN GARANTIZADA AL FINALIZAR
header("Location: importar.php");
exit;
?>